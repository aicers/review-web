...
fn from_filter_input(
    ...
    // Remove this allowance after resolving the TODO in the `else` block.
    #[allow(clippy::manual_map)]
    let sensors = if let Some(sensors_input) = &input.sensors {
        let map = store.node_map();
        Some(convert_sensors(&map, sensors_input)?)
    } else if ctx.data::<Role>() == Ok(&Role::SystemAdministrator) {
        None
    } else if let Some(customer_ids) = ctx
        .data::<String>()
        .ok()
        ...
        .and_then(|account| account.customer_ids)
    {
        ...
        // TODO: This block handles the case where the user doesn't have permission to view events.
        // Ideally, it should return `Some(Vec::new())` to exclude all events. However, doing so
        // currently breaks 10 test cases that do not account for sensor filtering. These test cases
        // should be updated first.

        // Normally, all accounts should belong to at least one customer, making this block
        // unreachable. However, during migration of previous account data, it's possible for an
        // account to temporarily have no associated customer.
        None
    };
...
mod tests {
    ...
    async fn event_list_suspicious_tls_traffic() {
        ...
        let fields = BlockListTlsFields {
            ...
            dst_port: 443,
            proto: 6,
            last_time: timestamp.timestamp_nanos_opt().unwrap(),

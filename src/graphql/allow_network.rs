...
async fn load(
    ...
    let store = crate::graphql::get_store(ctx)?;
    let map = store.allow_network_map();
    let additional_fields = AllowNetworkTotalCount {
    ...
    match customer_ids.as_deref() {
        ...
        Some(ids) => {
            ...
            super::load_edges_with_prefix_filtered(
                ...
                None,
                additional_fields,
                move |network: &review_database::AllowNetwork| {
...
mod tests {
    ...
    async fn test_allow_network() {
        ...
        let res = schema
            .execute_as_system_admin(r"{allowNetworkList(customerIds: [0]){totalCount}}")
            .await;
        ...
        assert_eq!(
            res.data.to_string(),
            r#"{allowNetworkList: {nodes: [{name: "Name 2", customerId: "0"}]}}"#
        );
    ...
    #[tokio::test]
    #[allow(clippy::too_many_lines)]
    async fn test_allow_network_multi_customer() {
        let schema = TestSchema::new().await;
        ...
        let res = schema
            ...
            .await;
        assert_eq!(res.data.to_string(), r#"{insertAllowNetwork: "1"}"#,);

        // Multi-customer query returns both, sorted by customer ID.
        let res = schema
            .execute_as_system_admin(
                r"query {
                    allowNetworkList(customerIds: [0, 1], first: 10) {
                        totalCount
                        nodes { name customerId }
                    }
                }",
        ...
        assert_eq!(
            res.data.to_string(),
            r#"{allowNetworkList: {totalCount: "2", nodes: [{name: "Net A", customerId: "0"}, {name: "Net B", customerId: "1"}]}}"#
        );
        ...
        let res = schema
            .execute_as_system_admin(
                r"query {
                    allowNetworkList(customerIds: [1, 0], first: 10) {
                        totalCount
                        nodes { name customerId }
                    }
                }",
        ...
        assert_eq!(
            res.data.to_string(),
            r#"{allowNetworkList: {totalCount: "2", nodes: [{name: "Net A"}, {name: "Net B"}]}}"#
        );
        ...
        let res = schema
            .execute_as_system_admin(
                r"query {
                    allowNetworkList(customerIds: [], first: 10) {
                        totalCount
                        nodes { name }
                    }
                }",
        ...
        let res = schema
            .execute_as_system_admin(
                r"query {
                    allowNetworkList(customerIds: [999]) {
                        totalCount
                    }
                }",
            )

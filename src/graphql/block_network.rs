...
impl BlockNetworkMutation {
    /// Inserts a new blocked network, returning the ID of the new blocked network.
    #[graphql(guard = "RoleGuard::new(Role::SystemAdministrator)
        .or(RoleGuard::new(Role::SecurityAdministrator))")]
    async fn insert_block_network(
        ...
        if let Some(network_list) = network_list
            && let Err(e) = apply_block_networks(ctx, &[network_list]).await
        {
            error_with_username!(ctx, "Failed to send  block networks: {e:?}");
        }
        Ok(ID(id.to_string()))
    }
    ...
    async fn remove_block_networks(
        ...
        let (removed, count, network_lists) = {
            ...
            if users_cids.is_some() {
                for &id in &ids {
                    let network = map.get_by_id(id)?.ok_or("no such block network")?;
                    if !customer_access::is_member(users_cids.as_deref(), network.customer_id) {
                        return Err(
                            "access denied: block network belongs to inaccessible customer".into(),
                        );
                    }
                }
            }
            ...
            let removed = ids
                ...
                .try_fold(Vec::with_capacity(count), |mut removed, id| {
                    if let Ok(Some(block_network)) = map.get_by_id(id)
                        && map.remove(id).is_ok()
                    {
                        let customer_id = block_network.customer_id;
                        let name = block_network.name;
    ...
    async fn update_block_network(
        ...
        let network_list = {
            let store = crate::graphql::get_store(ctx)?;
            let mut map = store.block_network_map();
            ...
            if !customer_access::is_member(users_cids.as_deref(), current_block_network.customer_id)
            {
                return Err("access denied: block network belongs to inaccessible customer".into());
            }
            ...
            {
                return Err(format!(
                    "Customer not found for current Blocklist (customerId: {customer_id})"
                )
                .into());
            }
            ...
            {
                return Err(format!(
                    "Customer not found for updated Blocklist (customerId: {customer_id})"
                )
                .into());
            }
...
mod tests {
    ...
    async fn test_block_network_multi_customer() {
        ...
        // Empty customerIds returns validation error.
        let res = schema
            .execute_as_system_admin(
                r"query {
    ...
    #[tokio::test]
    #[allow(clippy::too_many_lines)]
    async fn test_block_network_scoped_user() {
        let schema = TestSchema::new().await;
        ...
        let res = schema
            .execute_as_scoped_user(
                r"query {
                    blockNetworkList(customerIds: [1], first: 10) {
                        totalCount
                    }
                }",
                Role::SecurityAdministrator,
    ...
    #[tokio::test]
    #[allow(clippy::too_many_lines)]
    async fn test_block_network_mutation_scoping() {
        let schema = TestSchema::new().await;
        ...
        let res = schema
            .execute_as_scoped_user(
                &format!(
                    r#"mutation {{
                        removeBlockNetworks(ids: ["{c1_net_id}"])
                    }}"#
                ),
                Role::SecurityAdministrator,
                Some(vec![0]),
        ...
        let res = schema
            .execute_as_scoped_user(
                &format!(
                    r#"mutation {{
                        removeBlockNetworks(ids: ["{c0_net_id}", "{c1_net_id}"])
                    }}"#
                ),
                Role::SecurityAdministrator,
                Some(vec![0]),
        ...
        let res = schema
            .execute_as_scoped_user(
                &format!(
                    r#"mutation {{
                        removeBlockNetworks(ids: ["{c0_net_id}"])
                    }}"#
                ),
                Role::SecurityAdministrator,
                Some(vec![0]),
        ...
        assert_eq!(
            res.data.to_string(),
            r#"{removeBlockNetworks: ["Net Own Updated"]}"#
        );
    }
}

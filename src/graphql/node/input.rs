...
impl From<AgentInput> for review_database::Agent {
    fn from(input: AgentInput) -> Self {
        Self {
            ...
            config: match input.config {
                Some(config) => match config.try_into() {
                    Ok(config) => Some(config),
                    Err(_) => None,
                },
                None => None,
            },
            draft: match input.draft {
                Some(draft) => match draft.try_into() {
                    Ok(draft) => Some(draft),
                    Err(_) => None,
                },
                None => None,
...
pub(super) fn create_draft_update(
    ...
    Ok(review_database::NodeUpdate {
        ...
        profile_draft,
        agents: old.agents.clone().into_iter().map(Into::into).collect(),
        giganto: old.giganto.clone().map(Into::into),
    })
}

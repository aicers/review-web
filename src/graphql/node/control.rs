    ...
    if let Some(ref profile_draft) = update.profile_draft {
        ...
        if !new_hostname.is_empty() {
            ...
            for entry in map.iter(Direction::Forward, None) {
                ...
                // Check both current profile and profile_draft for hostname conflicts
                let existing_hostnames = [
                    existing_node.profile.as_ref().map(|p| &p.hostname),
                    existing_node.profile_draft.as_ref().map(|p| &p.hostname),
                ];

                for hostname_option in existing_hostnames.into_iter().flatten() {
                    if hostname_option == new_hostname {
                        return Err(format!(

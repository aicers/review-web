...
pub fn get_node_settings(db: &Store) -> Result<Vec<Setting>> {
    ...
    for res in map.iter(Direction::Forward, None) {
        ...
        let piglet: Option<ServerAddress> = if node_settings.piglet {
            Some(ServerAddress {
                web: None,
                // Set to the default value since the review address fields has been removed.
                rpc: Some(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), 0)),
        ...
        let reconverge = if node_settings.reconverge {
            Some(ServerAddress {
                ...
                // Set to the default value since the review address fields has been removed.
                rpc: Some(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), 0)),
                // Set to the default value since the giganto address fields has been removed.
                public: Some(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), 0)),
                ing: None,
            })
        } else {
            None

...
- Changed the behavior of the `EventListFilterInput::sensors` in event related
  ...
  - If the `sensors` field is provided, only events collected from the sensors
    are returned.
  - If the sensors field is not provided, the software filters events from the
    sensors owned by the customers to which the user belongs. An error occurs if
    the user does not belong to any customers.
